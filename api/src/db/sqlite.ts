...
class DatabaseConnection {
  ...
  public get<T = Record<string, unknown>>(sql: string, params: unknown[] = []): Promise<T | undefined> {
    return new Promise((resolve, reject) => {
...
class SQLiteHelper {
  ...
  public async connect(isTest: boolean = false): Promise<DatabaseConnection> {
    ...
    return new Promise((resolve, reject) => {
      this.connection = new Database(config.DB_FILE, (err) => {
        ...
        this.setupDatabase()
          .then(() => {
            const wrappedConnection = this.wrapConnection(this.connection || new Database(':memory:'));
  ...
  private async setupDatabase(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (config.FOREIGN_KEYS && this.connection) {
      ...
      if (config.ENABLE_WAL && config.DB_FILE !== ':memory:' && this.connection) {
